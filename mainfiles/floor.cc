...
// return the score the PC got
int floor::getScore() const {
    if (pc->getRace() == "Shade") {
        int score = (pc->get_gold() * 1.5);
        if (score != 0) {
            score += 1;
        }
        return score;
...
void floor::DragonGuardDragonHoard() {
    for (int c = 0; c < col; ++c) {
        for (int r = 0; r < row; ++r) {
            if (DragonHoard *dh = dynamic_cast<DragonHoard*>(grid[r][c].getOb())) {
                int dh_col = dh->get_col();
                int dh_row = dh->get_row();
                for (int col = dh_col - 1; col <= dh_col + 1; ++col) {
                    for (int row = dh_row - 1; row <= dh_row + 1; ++row) {
                        if (enemy *d = dynamic_cast<enemy*>(grid[row][col].getOb())) {
                            d->guardTreasure(dh);
                            dh->setDragon(d);
                        }
                    }
...
void floor::init(const string &mapName) {
    ...
    while (!map.eof()) {
        ...
        for (int c = 0; c < static_cast<int>(linetiles.length()); ++c, tempC = c) {
            ...
            obj->UpdatePos(theCell.getX(), theCell.getY());
            theCell.setOb(obj);
...
void floor::SpawnPlayer(const string &race) {
    ...
    this->pc->UpdatePos(playerCell->getX(), playerCell->getY());
    td->setPc(this->pc);
...
void floor::SpawnGold(const string &type) {
    ...
    while (num_gold < total_gold) {
        ...
        if (0 <= treasureID && treasureID <= 4) {
            ...
            t->UpdatePos(curRow, curCol);
            cell *DragonCell = getRandomCell(curRow, curCol);
...
void floor::pc_usePotion(const string &input) {
    ...
    map <string, cell*> theCell = get_dir_cell(curRow, curCol);
    Potion *p = dynamic_cast<Potion*>(theCell[input]->getOb());
    if (p) {
        pc->Take_Potion(p);
        td->updateAct("PC uses " + p->getName() + ". ");
        cell *potionCell = theCell[input];
        ...
        if (theEnemy) {
            ...
            if (theEnemy->get_hp() <= 0) {
                ...
                if (theEnemy->getRace() != "Human" && theEnemy->getRace() != "Merchant") {
                    ...
                    cell *DropTreasureCell = getRandomCell(theEnemy->get_row(), theEnemy->get_col());
                    ...
                    const int normal_hoard = 6;
                    const int total_normalhoard = 2;
                    td->updateAct("There are two Treasure appered. ");
                    
                    for (int num_normalhoard = 0; num_normalhoard < total_normalhoard;) {
                        cell *DropTreasureCell = getRandomCell(theEnemy->get_row(), theEnemy->get_col());
                        DropTreasureCell->rm_object();
                        DropTreasureCell->change_can_spawn(false);
...
void floor::Enemy_move(bool &freeze) {
    ...
    } // get the enemies from the map
    
    for (int i = 0; i < static_cast<int>(enemies.size());) {
        enemy *theEnemy = enemies[i];
        if (theEnemy->getRace() == "Dragon") {
            ++i;
            Dragon *dragon = dynamic_cast<Dragon*>(theEnemy);
            Treasure *dragon_hoard = dragon->getDragonHoard();
            if (nearby(pc, theEnemy->get_row(), theEnemy->get_col()) ||
                (dragon_hoard && nearby(pc, dragon_hoard->get_row(), dragon_hoard->get_col()))) {
                dragon->attack(pc);
            }
            ...
            if (nearby(pc, theEnemy->get_row(), theEnemy->get_col()) && theEnemy->is_hostile()) {
                ...
                cell *NewEnemyCell = getRandomCell(theEnemy->get_row(), theEnemy->get_col());
...
void floor::move_pc(const std::string &input) {
    ...
    if (moved) {
        ...
        if (move_ontoType == "/") {
            NewFloor = true;
            this->clear();
            this->init("map.txt"); // use default map as a base spawn objects;
...
// random get the spawnable cell that is used to spawn objects aorund the coordinates curRow and curCol
cell * floor::getRandomCell (const int &curRow, const int &curCol) {
    map <int, cell*> dir_cell;
    dir_cell[0] = &grid[curRow-1][curCol-1];
    dir_cell[1] = &grid[curRow-1][curCol];
    dir_cell[2] = &grid[curRow-1][curCol+1];
    dir_cell[3] = &grid[curRow][curCol-1];
    dir_cell[4] = &grid[curRow][curCol+1];
    dir_cell[5] = &grid[curRow+1][curCol-1];
    dir_cell[6] = &grid[curRow+1][curCol];
    dir_cell[7] = &grid[curRow+1][curCol+1];
    dir_cell[8] = &grid[curRow][curCol];
    int dir_key = rand() % 9;
    while (!dir_cell[dir_key]->CanSpawn()) {
        dir_key = rand() % 9;
    }
    return dir_cell[dir_key];
}

// random get the spawnable cell that is used to spawn objects
...
// ouput the ncurses view
void floor::ncursesView() {
    td->ncursesView();
}
